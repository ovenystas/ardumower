...
void RemoteControl::initSerial(int16_t baudrate)
{
    ...
    String unit, float scale, float maxvalue, float minvalue)
{
  ...
  Bluetooth.print(F(" `"));
  Bluetooth.print((static_cast<int16_t>(value / scale)));
  Bluetooth.print('`');
  Bluetooth.print((static_cast<int16_t>(maxvalue / scale)));
  Bluetooth.print('`');
  Bluetooth.print((static_cast<int16_t>(minvalue / scale)));
  Bluetooth.print(F("~ ~"));

  if (approximatelyEqual(scale, 10.0f, 1.0f))
  {
    Bluetooth.print(F("10"));
  }
...
void RemoteControl::sendADCMenu(bool update)
{
  ...
  for (uint8_t ch = 0; ch < 16; ch++)
  {
    ...
    Bluetooth.print(ch);
    Bluetooth.print(F(" min="));
    Bluetooth.print(adcMin);
    Bluetooth.print(F(" max="));
    Bluetooth.print(adcMax);
    Bluetooth.print(F(" diff="));
    Bluetooth.print(adcMax - adcMin);
    Bluetooth.print(F(" ofs="));
...
void RemoteControl::processErrorMenu(String pfodCmd)
{
  if (pfodCmd == "z00")
  {
    m_robot_p->resetAllErrorCounters();
...
void RemoteControl::sendMotorMenu(bool update)
{
  ...
  auto motorLeft_p = &m_robot_p->m_diffDrive.m_wheelLeft.m_motor;
  auto motorRight_p = &m_robot_p->m_diffDrive.m_wheelRight.m_motor;
  ...
  sendSlider("a07", F("Roll time max"), m_robot_p->m_diffDrive.m_rollTimeMax,
      "", 1, 8000);

  sendSlider("a19", F("Roll time min"), m_robot_p->m_diffDrive.m_rollTimeMin,
      "", 1, (m_robot_p->m_diffDrive.m_rollTimeMax - 500));

  sendSlider("a08", F("Reverse time"), m_robot_p->m_diffDrive.m_reverseTime,
      "", 1, 8000);

  sendSlider("a09", F("Forw time max"), m_robot_p->m_diffDrive.m_forwardTimeMax,
      "", 10, 80000);

  sendSlider("a12", F("Bidir speed ratio 1"),
      m_robot_p->m_diffDrive.m_biDirSpeedRatio1, "", 0.01, 1.0);

  sendSlider("a13", F("Bidir speed ratio 2"),
      m_robot_p->m_diffDrive.m_biDirSpeedRatio2, "", 0.01, 1.0);
...
void RemoteControl::processMotorMenu(String pfodCmd)
{
  ...
  else if (pfodCmd.startsWith("a03"))
  {
    ...
    motorLeft_p->setScale(motorLeft_p->getScale() /
        max(0.0f, static_cast<float>(motorLeft_p->getAverageSenseAdc())));
  }
  else if (pfodCmd.startsWith("a04"))
  {
    ...
    motorRight_p->setScale(motorRight_p->getScale() /
        max(0.0f, static_cast<float>(motorRight_p->getAverageSenseAdc())));
  }
  ...
  {
    processSlider(pfodCmd, m_robot_p->m_diffDrive.m_rollTimeMax, 1.0);
  }
  else if (pfodCmd.startsWith("a19"))
  {
    processSlider(pfodCmd, m_robot_p->m_diffDrive.m_rollTimeMin, 1.0);
  }
  else if (pfodCmd.startsWith("a08"))
  {
    processSlider(pfodCmd, m_robot_p->m_diffDrive.m_reverseTime, 1.0);
  }
  else if (pfodCmd.startsWith("a09"))
  {
    processSlider(pfodCmd, m_robot_p->m_diffDrive.m_forwardTimeMax, 10.0);
  }
  ...
  {
    processSlider(pfodCmd, m_robot_p->m_diffDrive.m_biDirSpeedRatio1, 0.01);
  }
  else if (pfodCmd.startsWith("a13"))
  {
    processSlider(pfodCmd, m_robot_p->m_diffDrive.m_biDirSpeedRatio2, 0.01);
  }
...
void RemoteControl::sendPerimeterMenu(bool update)
{
  ...
  {
    Bluetooth.print(F(" (inside)"));
  }
  else
  {
    Bluetooth.print(F(" (outside)"));
  }
...
void RemoteControl::sendOdometerMenu(bool update)
{
  ...
  Bluetooth.print(m_robot_p->m_odometer.m_encoder.left.getCounter());
  Bluetooth.print(F(", "));
  Bluetooth.println(m_robot_p->m_odometer.m_encoder.right.getCounter());

  Bluetooth.println(F("|l03~RPM Motor l, r "));
  Bluetooth.print(m_robot_p->m_odometer.m_encoder.left.getWheelRpmCurr());
  Bluetooth.print(F(", "));
  Bluetooth.println(m_robot_p->m_odometer.m_encoder.right.getWheelRpmCurr());
  ...
  sendYesNo(m_robot_p->m_odometer.m_encoder.left.m_swapDir);

  Bluetooth.print(F("|l06~Swap right direction "));
  sendYesNo(m_robot_p->m_odometer.m_encoder.right.m_swapDir);
...
void RemoteControl::processOdometerMenu(String pfodCmd)
{
  ...
  {
    TOGGLE(m_robot_p->m_odometer.m_encoder.left.m_swapDir);
  }
  else if (pfodCmd.startsWith("l06"))
  {
    TOGGLE(m_robot_p->m_odometer.m_encoder.right.m_swapDir);
  }
...
void RemoteControl::sendTimerDetailMenu(int16_t timerIdx, bool update)
{
  if (update)
  {
    Bluetooth.print(F("{:"));
...
void RemoteControl::sendCommandMenu(bool update)
{
  ...
  Bluetooth.print(m_robot_p->m_diffDrive.m_wheelLeft.m_motor.getPwmCur());
...
void RemoteControl::processCommandMenu(String pfodCmd)
{
  ...
  {
    m_robot_p->setNextState(StateMachine::STATE_MANUAL);
    m_robot_p->m_diffDrive.m_wheelLeft.m_motor.m_rpmSet += 10;
    m_robot_p->m_diffDrive.m_wheelRight.m_motor.m_rpmSet =
        -m_robot_p->m_diffDrive.m_wheelLeft.m_motor.m_rpmSet;
    sendCommandMenu(true);
  }
  else if (pfodCmd == "rk")
  {
...
void RemoteControl::run()
{
  ...
  if (m_pfodState == PfodState::LOG_SENSORS)
  {
    ...
    Bluetooth.print(',');
    Bluetooth.print(m_robot_p->m_diffDrive.m_wheelLeft.m_motor.getPowerMeas());
    Bluetooth.print(',');
    Bluetooth.print(m_robot_p->m_diffDrive.m_wheelRight.m_motor.getPowerMeas());
    Bluetooth.print(',');
    Bluetooth.print(m_robot_p->m_cutter.m_motor.getPowerMeas());
    Bluetooth.print(',');
    ...
    Bluetooth.print(m_robot_p->m_odometer.m_encoder.left.getCounter());
    Bluetooth.print(',');
    Bluetooth.print(m_robot_p->m_odometer.m_encoder.right.getCounter());
    Bluetooth.print(',');
  ...
  else if (m_pfodState == PfodState::PLOT_SENSOR_COUNTERS)
  {
    if (curMillis >= m_nextPlotTime)
    {
      ...
      Bluetooth.print(',');
      Bluetooth.print(m_robot_p->m_diffDrive.m_wheelLeft.m_motor.getOverloadCounter());
      Bluetooth.print(',');
      Bluetooth.print(m_robot_p->m_diffDrive.m_wheelRight.m_motor.getOverloadCounter());
      Bluetooth.print(',');
      Bluetooth.print(m_robot_p->m_cutter.m_motor.getOverloadCounter());
      Bluetooth.print(',');
  ...
  else if (m_pfodState == PfodState::PLOT_MOTOR)
  {
    if (curMillis >= m_nextPlotTime)
    {
      ...
      Bluetooth.print(m_robot_p->m_odometer.m_encoder.left.getWheelRpmCurr());
      Bluetooth.print(',');
      Bluetooth.print(m_robot_p->m_odometer.m_encoder.right.getWheelRpmCurr());
      Bluetooth.print(',');
      //      Bluetooth.print(robot->motorLeftSpeedRpmSet);
      Bluetooth.print(m_robot_p->m_diffDrive.m_wheelLeft.m_motor.m_pid.getSetPoint());
      Bluetooth.print(',');
      //      Bluetooth.print(robot->motorRightSpeedRpmSet);
      Bluetooth.print(m_robot_p->m_diffDrive.m_wheelRight.m_motor.m_pid.getSetPoint());
      Bluetooth.print(',');
      Bluetooth.print(m_robot_p->m_diffDrive.m_wheelLeft.m_motor.getPwmCur());
      Bluetooth.print(',');
      Bluetooth.print(m_robot_p->m_diffDrive.m_wheelRight.m_motor.getPwmCur());
      Bluetooth.print(',');
      Bluetooth.print(m_robot_p->m_diffDrive.m_wheelLeft.m_motor.m_pid.getErrorOld());
      Bluetooth.print(',');
      Bluetooth.println(m_robot_p->m_diffDrive.m_wheelRight.m_motor.m_pid.getErrorOld());
    }
  }
}
